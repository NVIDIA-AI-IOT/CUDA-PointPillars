...
def createNVOutput():
    for txt_i in range(0, 7481):
        str_i = str(txt_i).zfill(6)

        calib_path = '../data/kitti/training/calib/' + str_i + '.txt'
        image_2_path = '../data/kitti/training/image_2/' + str_i + '.png'
        velo_det_path = '../data/training_velo_reduce/box_prediction/result_' + str_i + '.txt'
        ...
        with open(cam_det_path, 'w') as the_file:
            for i in range(0, img_boxs.shape[0]):
                ...
                ry = conv(velo_det[i][6])
                alpha = -np.arctan2(x, z) + ry

                corners_2d = compute_box_3d(ry, w, l, h, [x, y, z], calib.P)        # swap w with h
                ...
                the_file.write(CLASS_NAMES[int(velo_det[i][7])])
                the_file.write(" 0.00")             # 是否被截断
                the_file.write(" 0")                # 是否被遮挡
                the_file.write(f" {alpha:.2f}")     # alpha，物体的观察角度，范围：-pi~pi

                the_file.write(f" {xy_min[0]:.2f} {xy_min[1]:.2f} {xy_max[0]:.2f} {xy_max[1]:.2f} ")    # xmin，ymin，xmax，ymax
                the_file.write(f"{h:.2f} {l:.2f} {w:.2f} ")                                             # h l w
                the_file.write(f"{x:.2f} {y:.2f} {z:.2f} ")                                             # x y z

                the_file.write(f"{ry:.2f} ")
                the_file.write(f"{velo_det[i][8]:.2f}")
                the_file.write("\n")
...
def createPCDetOutput():
    with open("./result.pkl", "rb") as f:
        ...
        for txt_i in range(0, len(dt_annos)):
            ...
            cam_det_path = './pcdet_output/' + frame['frame_id'] + '.txt'
            with open(cam_det_path, 'w') as the_file:
